...
describe('parseMultipartRelated — FR-022 truncation (T-037, T-066)', () => {
  ...
  it('T-037: source ends before closing boundary → MultipartTruncatedError; cleanup runs', async () => {
    ...
    // Either MultipartTruncatedError (our preferred outcome) OR a parser
    // error fires — the queue is first-fire-wins. T-037 is
    ...
    // parser error has a chance to propagate.
    ...
    if (captured instanceof MultipartTruncatedError) {
      ...
      // Parser-derived parse error — also acceptable per the planner's
  ...
  it('T-066: partial part header followed by stream end → either truncation or parser parse error; cleanup intact', async () => {
    // The envelope ends mid-part-header (no terminating CRLF), simulating
    // a connection cut during header read. Per the spec, either
    // MultipartTruncatedError OR a parser-derived parse error is acceptable
    ...
    try {
      ...
      // whether parser surfaces the partial header before source 'end'.
      while (true) {
    ...
    // SOME error must surface (either ours or parser's) — the partial
  ...
  it('truncation push fires when source ends and parser has not finished (line-298 coverage)', async () => {
    // Hand-craft a partial envelope where the last `--BOUNDARY` is left
    // hanging without `--` (so parser is mid-state when source ends).
    // This is the cleanest path to exercise the truncation push code line
    // — the parser 'error' DOESN'T fire first because the per-part body
    ...
    try {
      ...
      while (true) {
        const next = await iter.next();
        if (next.done) break;
        // Drain part body to release parser's internal state machine.
    ...
    expect(captured).toBeInstanceOf(Error);
    // Either MultipartTruncatedError OR a parser-derived parse error.
    await settle();
    expect(source.destroyed).toBe(true);
  });
  ...
  it('truncation: bytesReceived field reflects actual envelope size when MultipartTruncatedError fires', async () => {
    ...
    let captured: unknown;
    try {
      // Drain every part body to completion so parser's 'finish' has had
      // every chance to fire IF the closing boundary were present. With
      // closingBoundary: false, parser cannot finish — source 'end' wins
